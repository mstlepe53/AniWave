...
export function useHomeData() {
  return useQuery({ queryKey: ['anime-home'], queryFn: getHomeData, staleTime: TTL.home, gcTime: TTL.home*2, retry: 1 });
}

export function useAnimeDetails(id: string | number) {
  return useQuery({ queryKey: ['anime', String(id)], queryFn: () => getAnimeDetails(id), staleTime: TTL.detail, gcTime: TTL.detail*2, retry: 1, enabled: !!id });
}
...
export function useAnimeList(type: AnimeListType, genre: string|undefined, page: number) {
  return useQuery<AnimeCard[]>({
    ...
    staleTime: TTL.list, gcTime: TTL.list*2, placeholderData: keepPreviousData, retry: 1,
  });
}
...
export function useAnimeSearch(q: string) {
  return useQuery({
    ...
    staleTime: TTL.search, gcTime: TTL.search*2, enabled: !!q.trim(), retry: 1,
  });
}
