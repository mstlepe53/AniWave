...
export default function AnimeDetail() {
  ...
  const totalEps = anime.episodes || (anime.nextAiringEpisode ? anime.nextAiringEpisode.episode - 1 : 0) || 12;
    ...
    <div className="max-w-[1600px] mx-auto p-4 md:p-6 space-y-10">
        ...
        <div className="relative z-10 p-6 md:p-12 flex flex-col md:flex-row gap-8">
          ...
          <div className="w-full md:w-52 shrink-0 space-y-3">
            ...
            <div className="flex gap-2">
              <Link to={watchUrl} onClick={() => addToHistory({title, image: posterImg, url: watchUrl, subtitle: 'Ep 1'})}
                className="flex-1 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-bold flex items-center justify-center gap-2 active:scale-95 transition-all text-sm">
              ...
              <button onClick={handleToggleWatchlist} disabled={!token||wlLoading}
                className={`w-11 h-11 rounded-lg flex items-center justify-center transition-all active:scale-95 shrink-0 ${watchlisted?'bg-indigo-500 text-white':'bg-white/10 text-white hover:bg-white/20'} disabled:opacity-50`}>
          ...
          <div className="flex-1 text-white min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-2">
              <span className="px-2 py-0.5 text-xs font-bold rounded bg-indigo-600">ANIME</span>
            ...
            <div className="flex flex-wrap items-center gap-3 mb-4 text-sm">
              ...
              {totalEps > 0 && <span className="flex items-center gap-1 text-gray-300"><Tv className="w-4 h-4"/>{totalEps} Episodes{anime.nextAiringEpisode?` (${anime.nextAiringEpisode.episode-1} aired)`:''}</span>}
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {(anime.genres||[]).map(g=>(
                <Link key={g} to={`/genre/${encodeURIComponent(g)}`} className="px-3 py-1 bg-white/10 hover:bg-indigo-600 rounded-full text-xs font-bold transition-colors">{g}</Link>
              ))}
            </div>
            ...
            {desc && (
              <div className="mb-4">
                ...
                {desc.length > 200 && <button onClick={()=>setShowFull(!showFull)} className="text-indigo-400 text-xs mt-1 flex items-center gap-1 hover:text-indigo-300">{showFull?<><ChevronUp className="w-3 h-3"/>Less</>:<><ChevronDown className="w-3 h-3"/>More</>}</button>}
              </div>
            )}
      ...
      {totalEps > 0 && (
        <div className="space-y-4">
          <h2 className="text-lg font-bold dark:text-white flex items-center gap-2"><Tv className="w-5 h-5 text-indigo-500"/>Episodes</h2>
            ...
            {Array.from({length: Math.min(totalEps, 500)}).map((_, i) => {
              ...
              return (
                <Link key={ep} to={epUrl}
                  ...
                  className="group relative aspect-video rounded-lg overflow-hidden bg-gray-800 border border-gray-700 hover:border-indigo-500 transition-colors">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2">
            {cast.map(e => (
              <div key={e.node.id} className="shrink-0 w-24 text-center">
                ...
                <p className="text-[10px] text-indigo-400 capitalize">{e.role?.toLowerCase()}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3">
            {anime.relations.edges.filter((e:any)=>e.node.type==='ANIME').map((e:any) => (
              <div key={e.node.id} className="flex flex-col gap-1">
                ...
                <span className="text-[10px] font-bold text-indigo-500 text-center uppercase truncate px-1">
