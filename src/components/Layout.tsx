...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 transition-colors">
      {/* Header */}
      <header className="sticky top-0 z-30 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border-b border-gray-200 dark:border-gray-800 h-16">
          ...
          <nav className="hidden md:flex items-center gap-1 ml-4">
            {NAV_LINKS.map(l => (
              <Link key={l.to} to={l.to}
                className="px-3 py-1.5 text-sm font-semibold text-gray-600 dark:text-gray-300 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-lg transition-colors">
                {l.label}
              </Link>
            ))}
          </nav>
            ...
            <form onSubmit={handleSubmit} className="w-full">
              <div className="relative">
                ...
                <input type="text" placeholder="Search anime..." value={query}
                  ...
                  className="w-full bg-gray-100 dark:bg-gray-800 border border-transparent focus:border-indigo-500 dark:focus:border-indigo-400 rounded-xl py-2 pl-9 pr-4 text-sm outline-none transition-all dark:text-white dark:placeholder-gray-500" />
              </div>
            </form>
                ...
                {query.trim() ? (
                  ...
                  suggestions.map(s => (
                    <button key={s.id} onMouseDown={() => handleSuggestionClick(s.id, s.title)}
                      ...
                      <Tv className="w-3.5 h-3.5 text-indigo-400 shrink-0 ml-auto" />
                    </button>
                  ))
                ) : (
                  <div className="p-2">
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            {user ? (
              <div className="relative group">
                <button className="w-9 h-9 rounded-full overflow-hidden border-2 border-indigo-400 hover:border-indigo-500 transition-colors">
              ...
              <Link to="/login" className="w-9 h-9 rounded-full bg-gray-800 dark:bg-gray-700 flex items-center justify-center hover:bg-gray-700 dark:hover:bg-gray-600 transition-colors border-2 border-gray-600 dark:border-gray-500">
          ...
          <nav className="md:hidden absolute top-16 left-0 right-0 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 shadow-xl z-50">
            {NAV_LINKS.map(l => (
              <Link key={l.to} to={l.to} onClick={() => setMobileMenu(false)}
                className="flex items-center gap-2 px-4 py-3 text-sm font-semibold text-gray-700 dark:text-gray-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 hover:text-indigo-600 border-b border-gray-100 dark:border-gray-800 last:border-0">
                {l.label}
              </Link>
            ))}
          </nav>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 flex">
        {[
          ...
          <Link key={item.to} to={item.to}
            className="flex-1 flex flex-col items-center gap-0.5 py-2.5 text-gray-500 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors text-[10px] font-bold">
      ...
      <footer className="hidden md:block bg-gray-900 text-gray-400 py-10 mt-10">
        <div className="max-w-[1600px] mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2 text-white font-black text-xl">
                ...
                Anime<span className="text-indigo-400">Vault</span>
              </div>
          ...
          <div className="mt-8 pt-6 border-t border-gray-800 text-center text-xs text-gray-600">
            <p>AnimeVault does not host any files. All content is sourced from third-party embed providers.</p>
            <p className="mt-1">Anime data powered by <a href="https://anilist.co" target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:underline">AniList</a> GraphQL API.</p>
          </div>
        </div>
      </footer>
